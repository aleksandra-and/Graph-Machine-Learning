...
best_model.load_state_dict(torch.load('best_model_TGT_gat_tcn_A_fixed2.pt'))
...
def simulate_trading(stock_data, initial_capital, stop_loss_threshold=0.95):
    """
    模拟一日股票交易过程
    """
    ...
    for index, row in stock_data.iterrows():
        ...
        # 计算实际价格（用于止损判断）
        true_price = gourujia * (1 + earnings_rate / 100)

        # ===== 止损逻辑 =====
        # 如果真实价格跌破止损阈值，强制按止损价卖出
        if true_price < gourujia * stop_loss_threshold:
            # 使用止损价计算卖出金额
            sell_price = gourujia * stop_loss_threshold
            sell_amount_true = shares_dict[stock_index] * sell_price
        else:
            # 正常卖出金额
            sell_amount_true = shares_dict[stock_index] * true_price
...
test_daily_rates_df.to_csv('../output/test_earnings_rate1.csv', index=False, encoding='gb18030')
...
test_cumulative_rates_df.to_csv('../output/test_cumulative_rates1.csv', index=False, encoding='gb18030')
